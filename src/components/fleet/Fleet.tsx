...
import { useRef } from "react"; // added for auto scroll
...
export const FleetPage = () => {
  // Ref for the content section to scroll into view on mobile
  const contentRef = useRef<HTMLDivElement>(null);

  const handleTabChange = () => {
    // Only scroll on small devices (window width < 768px)
  ...
  return (
    <section className="min-h-screen flex flex-col">
      <div className="flex-grow px-4 pt-32 pb-0 md:pb-10 md:px-[5%] lg:pt-40 2xl:px-[10%]">
        <Tabs defaultValue="megacaravan" className="w-full" onValueChange={handleTabChange}>
          <div className="mb-6 md:mb-4">
            <div className="w-full lg:w-1/2 2xl:w-full 2xl:max-w-[56rem]">
              <h1 className="mb-6 font-dmsans text-4xl font-bold text-[#094d82] md:text-6xl md:font-normal lg:text-7xl lg:font-bold">
                FLEET INFORMATION
              </h1>
            ...
            {/* Mobile Description: hidden on md+ */}
            <FleetDescription className="mt-16 space-y-5 text-justify font-medium md:hidden" />
          </div>

          {/* Scroll Target Div for Mobile */}
          <div ref={contentRef} className="scroll-mt-24 md:scroll-mt-0" />
              ...
              <TabsContent value={tab} className="mt-0 2xl:mt-2" key={tab}>
                ...
                >
                  <div className="grid grid-cols-1 gap-5 md:gap-6 lg:grid-cols-5 lg:gap-x-10 lg:gap-y-3 xl:gap-y-2">
                    ...
                    <FleetDescription className="hidden space-y-4 text-justify font-medium md:col-span-2 md:block lg:self-start lg:pt-1" />
                  </div>
