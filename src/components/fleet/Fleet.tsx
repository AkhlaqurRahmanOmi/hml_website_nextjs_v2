...
type FleetKey = "megacaravan" | "megacaravan2" | "megatrust" | "megapassion" | "cydf";
    ...
    <section className="min-h-screen flex flex-col">
      <div className="px-4 md:px-[5%] 2xl:px-[10%] pt-32 lg:pt-40 pb-10 flex-grow">
        <Tabs defaultValue="megacaravan" className="w-full">
          <div className="grid grid-cols-1 xl:grid-cols-5 gap-10 items-start mb-24 md:mb-10">
            ...
            <div className="col-span-1 lg:col-span-3">
              ...
              <TabsList className="grid grid-cols-2 md:grid-cols-5 md:tracking-wide gap-2">
                <TabsTrigger value="megacaravan" className="flex flex-col items-center lg:text-md font-semibold">
                  MEGA CARAVAN <span className="text-[12px]">16,200mt</span>
                </TabsTrigger>
                <TabsTrigger value="megacaravan2" className="flex flex-col items-center lg:text-md font-semibold">
                  MEGA CARAVAN 2 <span className="text-[12px]">17,700mt</span>
                </TabsTrigger>
                <TabsTrigger value="megatrust" className="flex flex-col items-center lg:text-md font-semibold">
                  MEGA TRUST <span className="text-[12px]">19,200mt</span>
                </TabsTrigger>
                <TabsTrigger value="megapassion" className="flex flex-col items-center lg:text-md font-semibold">
                  MEGA PASSION <span className="text-[12px]">52,300mt</span>
                </TabsTrigger>
                <TabsTrigger value="cydf" className="flex flex-col items-center lg:text-md font-semibold">
                  CYDF <span className="text-[12px]">25,163mt</span>
                        ...
                        {/* Description */}
            <div className="col-span-1 md:grid-cols-1 lg:col-span-2 font-medium space-y-4 mt-10 md:mt-0 xl:mt-48 text-justify">
              <p>
                At HML, we deliver flexible and cost-effective transport solutions by leveraging the
                expansive deck space of our vessels, maximizing cargo capacity in a single shipment.
                Designed with high deck load strength and an open-stern configuration, our vessels
                enable loading and discharging from both the stern and sides. A key advantage of the
                HML fleet is the shallow vessel draft, allowing access to ports with limited water
                depth—eliminating the need for spacer barges or additional floating equipment.
              </p>
              <p>
                All vessels operated by HML adhere to international ship and crew management
                standards, including ISM, OSAS, and CMID. They are asbestos-free and meet the
                highest quarantine and environmental regulations, ensuring safe, efficient, and
          ...
          {/* Tab Content */}
          {(["megacaravan", "megacaravan2", "megatrust", "megapassion", "cydf"] as FleetKey[]).map((tab) => {
            const fleetData = FleetData.find((f) => f.id === tab);
            if (!fleetData) return null;

            return (
              <TabsContent value={tab} className="mt-6 2xl:mt-10 space-t-10" key={tab}>
                <motion.div variants={slideFadeIn} initial="hidden" animate="visible" exit="hidden">
                    ...
                    <aside className="w-full lg:w-2/4 space-y-2">
                      {[
                        ...
                        <div key={label}>
                          ...
                          {(index + 1) % 2 === 0 && <hr className="border-dashed my-1.5" />}
                        </div>
                      ))}
                    </aside>
                  </div>
